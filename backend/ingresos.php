...
function listar()
{
    ...
    $sql = "select * from ingresos where usuario = ?";
            ...
            <div class="swiper border-b mb-3 border-gray-200">
                <div class="swiper-wrapper pb-3">
                    <div class="swiper-slide menu">
                        <button onclick="eliminarIngreso('<?php echo $row['id']; ?>')" class="w-10 h-10 rounded-lg bg-red-100 text-red-900 hover:bg-red-200 focus:bg-red-200 flex items-center justify-center me-3 text-2xl">
...
function insert(){
    if ($_POST['token'] != "" && $_POST['token'] == $_SESSION['token']) {
        if ($_POST['detalle'] != "" && $_POST['dinero'] != "" && $_POST['fecha'] != "") {
            ...
            $dinero = $_POST['dinero'];
            $fecha = $_POST['fecha'];
            $sql = "insert into ingresos (detalle, fecha, dinero, usuario, estado) values (?, ?, ?, ?, 1)";
            $stmt = mysqli_prepare($conexion, $sql);
...
function delete(){
    if($_POST['token'] != "" && $_POST['token'] == $_SESSION['token']){
        if($_POST['id'] != ""){
            ...
            $id = $_POST['id'];
            $userid = $_SESSION['userid'];
            $sql = "delete from ingresos where id = ? and usuario = ?";
            $stmt = mysqli_prepare($conexion, $sql);
            mysqli_stmt_bind_param($stmt, "ii", $id, $userid);
            mysqli_stmt_execute($stmt);
            if(mysqli_stmt_affected_rows($stmt) > 0){
                echo json_encode(array('response' => 'success', 'message' => 'Ingreso eliminado correctamente'));
            }else{
                echo json_encode(array('response' => 'error', 'message' => 'Error al eliminar el Ingreso'));
            }
        }else{
            echo json_encode(array('response' => 'error', 'message' => 'Faltan datos'));
        }
